...
// document.body.appendChild(pElem, pElem_2) // prepend // after // before
//& appendChild добавляет всего лишь один элемент и возвращает его // append добавляет несколько элементов и ничего не возвращает
//^^ 'beforebegin': до самого element (до открывающего тега). // 'afterbegin': сразу после открывающего тега element (перед первым потомком).
//^^ '⁡⁣⁣⁢beforeend⁡': сразу перед закрывающим тегом element (после последнего потомка). // 'afterend': после element (после закрывающего тега).
...
// [Самостоятельно] Создайте функцию createThumbnailsContainer, которая создает элемент div для изображений, добавляет класс, добавляет его в тело документа и возвращает этот элемент.
...
// [Самостоятельно] Создайте функцию createImgThumbnail, которая принимает один аргумент - ссылку на изображение, создает элемент div, содержащий изображение с заданным источником и возвращает его.
...
// [Самостоятельно] Создайте функцию initializeThumbnails, которая принимает два аргумента, массив ссылок на изображения и элемент div для изображений, получаемый с помощью createThumbnailsContainer, потом заполняет этот див элементами изображения, используя createImgThumbnail.
// initializeThumbnails(srcArray, thumbnailsContainer)
// на основе ссылок из srcArray с помощью функции createImgThumbnail создать контейнер картинок
...
function initializeThumbnails(srcArray, thumbnailsContainer) {
  for (let i = 0; i < srcArray.length; i++) {
    // [Самостоятельно] Модифицируйте функцию initializeThumbnails так, чтобы при клике по элементу изображения вызывалась функция setActiveImage со ссылкой той картинки, на которую нажали.
...
// [Самостоятельно] Создайте функцию setActiveImage, которая в параметрах получает ссылку на картинку, создает контейнер активного изображения и добавляет в DOM.
// setActiveImage(src)
...
// [Самостоятельно] Создайте функцию unsetActiveImage, которая получает из DOM и удаляет элемент активного изображения.
...
// [Вместе] Создайте функции nextImage и prevImage, которые будут переключать активную картинку.
// ["img1", "img2", "img3"] // 0 -1
function nextImage() {
  ...
  if (activeImageIndex >= images.length) {
    // если индекс выходит за пределы массива с правой стороны то начинаем с первого элемента
    activeImageIndex = 0;
  }
...
function prevImage() {
  ...
  activeImageIndex = activeImageIndex - 1;
  if (activeImageIndex < 0) {
    // если индекс выходит за пределы массива с левой стороны то начинаем с последнего элемента
    activeImageIndex = images.length - 1;
  }
  setActiveImage(images[activeImageIndex]);
}
// [Вместе] При нажатии клавиши Esc вызвать функцию unsetActiveImage, нажатии стрелки вправо - nextImage, влево - prevImage.
