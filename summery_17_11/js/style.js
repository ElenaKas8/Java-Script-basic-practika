...
//&ОБЪЕКТЫ
//*Написать код, где есть целочисленный массив и обработать каждое число из этого массива. Вывести в консоли массив, обработав каждое число из входного массива следующим образом:Если число имеет целочисленный квадратный корень, то извлечь его, в противном случае возвести число в квадрат. Пример:[4,3,9,7,2,1] -> [2,9,3,49,4,1]


//&!!возвести число в квадрат-4**0.5
// =================== tasks ===================
...
for (let i = 0; i < arr.length; i++) {
  ...
  if (sqrt === roundedSqrt) {
    // если да значит там целочисленный квадратный корень
    resultArr.push(sqrt);
  } else {
    // если нет значит там квадратный корень число с плавующей точкой
...
//*Написать код, где будет массив полный мусора, но содержащий одну "иголку".Найти “иголку”, вывести в консоли сообщение, которое будет содержать:"найдена иголка в позиции {индекс}”, по которому она была найдена.Пример:["сено", "мусор", "сено", "сено", "песок", "иголка", "трава"]. --> "найдена иголка в позиции 5"
...
//   for (let i = 0; i < arr.length; i++) {
    //Проверяем, является ли текущий элемент "иголкой"
    // if (arr[i] === "иголка") {
      //Если да, выводим сообщение и завершаем цикл
    //   console.log(`Найдена иголка в позиции ${i}`);
    //   return;
    // }
//   }
// 
  //Если "иголка" не найдена
//   console.log("Иголка не найдена в массиве");
// };
// 
//Вызываем функцию
// findNeedle(array);
...
// Функция для поиска "иголок"
const findNeedles = (arr) => {
  arr.forEach((innerArray, outerIndex) => {
    innerArray.forEach((element, innerIndex) => {
      if (element === "иголка") {
        console.log(`Найдена иголка в позиции [${outerIndex}, ${innerIndex}]`);
      }
    });
  });
};
...
//&Создать массив users из объектов с данными пользователей (свойства id, firstname, lastname, age и country). Данные взять произвольные.Пройдитесь циклом по массиву users и сформируйте строки с описанием каждого совершеннолетнего пользователя. Выведите каждую полученную строку в консоль
